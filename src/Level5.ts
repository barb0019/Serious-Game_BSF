...
export default class Level5 extends Level {
  ...
  protected speedbubbles(game: Game): void {
    this.speedBubble = [];
    this.speedBubble.push(new SpeedBubble(game, 'In veel landen zijn het kopen van lootboxen verboden', this.game.canvas.height * 0.55, this.game.canvas.width * 0.05, 100, 500));
    this.speedBubble.push(new SpeedBubble(game, 'Van lootboxen krijg je vaak niets nuttigs', this.game.canvas.height * 0.01, this.game.canvas.width * 0.6, 100, 500));
    this.speedBubble.push(new SpeedBubble(game, 'van teveel in-game aankopen kan je in geld problemen komen', this.game.canvas.height * 0.3, this.game.canvas.width * 0.75, 100, 500));
  }
